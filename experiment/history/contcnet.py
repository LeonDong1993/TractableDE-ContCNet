...
nn_conf.init_epoch = 7
nn_conf.last_epoch = 7
nn_conf.max_epoch = 150
nn_conf.weight_decay = 1e-4
nn_conf.corr_thresh = 0.05
nn_conf.drop_prob = 0.5
...
class ContCNet:
    ...
    def _predict_(self, z, query, N = -1, smooth = False):
        ...
        ux_ratio = len(ux) / max(len(self.xids), 10)
        N = int(N * (1 + ux_ratio*9))
        extra_sample_rate = (1 + ux_ratio)
