...
#        lp_current_sum = (lp_current.sum(1) * D / (D-d)).sum() #1
    ...
    # likelihood weighting sampling, by leon
    def lw_sample(self, evi = None, n=1):
        if evi is None:
            known = []
            x = None
            evi = (known,x)
        ...
        # known is a list(array-like) contains the ids of known variables
        # x is a list(array-like) contains the value of all evidence variables
        # e.g. known = [0,3] , x = [1.2, None,None, 1.4, None] (5 total variable)
        ...
        for s in xrange(n):
            ...
            for j in xrange(self.n_visible):
                ...
                if i in known:
                    samples[i, s] = x[i]
                    this_weight *= logsumexp(-0.5 * ((Mu - samples[i, s]) / Sigma) ** 2 - np.log(Sigma) - 0.5 * np.log(2 * np.pi) + np.log(Alpha))
                else:
                    comp = Utils.nnet.random_component(Alpha)
                    samples[i, s] = np.random.normal(Mu[comp], Sigma[comp])
                if j < self.n_visible - 1:
            ...
            sample_weight.append(this_weight)

        # every row is a sample generated, by leon
        return samples.T, sample_weight
