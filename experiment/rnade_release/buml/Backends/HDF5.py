...
class HDF5(Backend):
    def __init__(self, path, filename, comment="", read = False):
        self.path = path
        ...
        if not read:
            ...
            self.f = h5py.File(os.path.join(self.path, self.filename), 'r')
...
#        return h5py.File(os.path.join(self.path, self.filename), 'a')
    ...
    def write_helper(self, f, route, attribute, value):
        ...
        rs = "/" + '/'.join(map(str, route + [attribute])) # leon fix support python 3
        try:
            try:
                del f[rs]
            except:
